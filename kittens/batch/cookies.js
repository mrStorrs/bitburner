    ...
    while (true) {
        ...
        for (let server of servers){
            ...
            if (!batchingStarted && (serverStats.security > 2 || serverStats.money > 10000)) {
                let threadsAvailable = Math.floor(serverStats.availableRam / ns.getScriptRam("/kittens/batch/prep.js"));
                if (server == "home") threadsAvailable = threadsAvailable - 20; 
            ...
            // } else if (batchingStarted && (serverStats.money > (serverStats.maxMoney * hackPercent + serverStats.maxMoney * .1) || serverStats.security > ns.getServerMinSecurityLevel(targetServer) + (((hackPercent / ns.hackAnalyze(targetServer)) * .002) + 1 ))){
                ...
                for (let i = 0; i < scripts.length; i++) {
                    ...
                    await ns.exec(script, server, threads, targetServer, delay, "batch-" + batchIdx, workerDebug);
                }
            ...
            await ns.sleep(spacer * 4.1); //techincally i think this could be * 4 but this will give us some extra space. 
        }
...
function getTarget(ns, currentTarget, hackLevel){
    ...
    if (hackLevel > 600 && ns.hasRootAccess("the-hub") && hamRam > 4000){
        newTarget = "the-hub";
        if(hamRam){
            hackPercent = 0.95
        }
